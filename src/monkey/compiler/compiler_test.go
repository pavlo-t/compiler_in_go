...
func TestHashLiterals(t *testing.T) {
	tests := []compilerTestCase{
		...
		},
		{
			...
			expectedConstants: []interface{}{1, 2, 3, 4, 5, 6, 7, 8},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
				code.Make(code.OpAdd),
...
func testConstants(
	...
	for i, constant := range expected {
		...
		case []code.Instructions:
			actualInstructions, ok := actual[i].(*object.CompiledFunction)
			if !ok {
				return fmt.Errorf("constant %d - not a CompiledFunction: %T", i, actual[i])
			}
			err := testInstructions(constant, actualInstructions.Instructions)
			if err != nil {
